...
fn create_display(
    ...
    if !cfg!(target_os = "windows") {
        ...
        if let Some(window_settings) = &window_settings {
            window_settings.restore_positions(&gl_window);
        }
    }
...
pub fn run(mut app: Box<dyn epi::App>, native_options: epi::NativeOptions) {
    ...
    let gl_window = create_display(&*app, &native_options, window_settings, icon, &event_loop);

    let repaint_signal = std::sync::Arc::new(GlowRepaintSignal(std::sync::Mutex::new(
        event_loop.create_proxy(),
    )));

    let mut egui = EguiGlow::new(&gl_window);
        ...
        if running {
...

            {
                ...
                unsafe {
                    ...
                    egui.painter.gl.finish();
                    // here
                    egui.painter.gl.flush();
                }
            ...
            if let Some(storage) = &mut storage {
                ...
                if now - last_auto_save > app.auto_save_interval() {
                    epi::set_value(
                        storage.as_mut(),
                        WINDOW_KEY,
                        &WindowSettings::from_display(&gl_window),
                    );
